...
namespace Handlr.Framework.Routing.Steps
{
    ...
    public class AdoCall : DatabaseCall
    {
        ...
        public override IFieldCache ExecuteStep(IFieldCache fieldCache)
        {
            ...
            using (var db = Core.Factory<Ado>(ParseValue(LoaderArguments.ConnectionString) as string, null, false, "", true))
            {
                ...
                {
                    throw new ParserException(string.Format("The output from the ADO call could not be parsed using the post translation of type \"{0}\": {1}", LoaderArguments.OutputTranslation.GetType(), ex.Message));
                }
            }
            return fieldCache;
        }
