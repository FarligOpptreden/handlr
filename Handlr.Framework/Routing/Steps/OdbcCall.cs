...
namespace Handlr.Framework.Routing.Steps
{
    ...
    /// </summary>
    [Tag("DatabaseCall")]
    public class OdbcCall : DatabaseCall
    {
        /// <summary>
        /// Creates a new OdbcCall instance.
        /// </summary>
        /// <param name="executionContext">The current execution context of the step</param>
        public OdbcCall(IController executionContext) : base(executionContext) { }
        ...
        public override IFieldCache ExecuteStep(IFieldCache fieldCache)
        {
            ...
            using (var db = Core.Factory<Odbc>(ParseValue(LoaderArguments.ConnectionString) as string, null, false, "", true))
            {
                ...
                {
                    throw new ParserException(string.Format("The output from the ODBC call could not be parsed using the post translation of type \"{0}\": {1}", LoaderArguments.OutputTranslation.GetType(), ex.Message));
                }
            }
            return fieldCache;
        }
